...
// errado
db.clientes.aggregate([{ $group: { _id: { "$endereco.uf": "ES" }, total: { $sum: 1 } } }]);

// certo sem chave no _id, porem retorna todos os estados agrupados
...
db.vendas.aggregate([{ $group: { _id: "$clienteId",  valorTotal: {  $sum: "$valorTotal" } } }, { $sort: { valorTotal: -1 } }, { $limit: 5 }]);
...
db.vendas.aggregate([{ $match: { dataVenda: { $gte: ISODate("2019-01-01"), $lte: ISODate("2019-12-31") } } }, { $group: { _id: "$clienteId",  valorTotal: {  $sum: "$valorTotal" } } }, { $sort: { valorTotal: -1 } }, { $limit: 10 }]);
...
// gabarito
db.vendas.aggregate([
  { // agrupa clientes por ID e conta esses cliente por grupo
    $group: {
      _id: "$clienteId",
      totalCompras: {
        $sum: 1
  ...
  { // filtra somente os grupos com comprar maiores que 5
    $match: {
      totalCompras: { $gt: 5 }
    }
  },
  { // conta quantos grupos tem
    $group: {
      _id: null,
      clientes: { $sum: 1 }
    }
  },
  { // projeta somente o campo clientes
    $project: { _id: 0 }
  }
]);
...
db.vendas.aggregate([
  ...
  { // operador conta todos os grupos
    $count: "clientes"
  }
]);
